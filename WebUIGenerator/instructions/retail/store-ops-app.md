...
**Sidebar label:** "Performance"
**Component:** `src/pages/PerformanceLab.tsx`

Advanced retail analytics — visual deep-dive into store and promo performance.

- **Funnel chart (top-left, 340px):**
  - Customer conversion funnel: Foot Traffic → Browsing (70%) → Basket (40%) → Purchase (28%) → Loyalty Signup (8%)
  - Derive from hourly_metrics: traffic → transactions conversion ratio
  - Show % drop-off at each stage

- **Radar chart (top-right, 340px):**
  - Compare store formats across 5 dimensions: Traffic/sqft, Revenue/sqft, Avg Basket, Satisfaction, Staff Efficiency
  - One polygon per format (Flagship, Standard, Express, Outlet, Pop-up)
  - Normalize to 0–100 scale

- **Waterfall chart (middle, full width, 300px):**
  - Revenue bridge: Base Revenue → Promotion Uplift → Seasonal Effect → New Stores → Closures → Net Revenue
  - Green for additions, red for subtractions, purple for total
  - Dollar labels on each bar

- **Polar/rose chart (middle-left, 320px):**
  - Revenue by hour of day (14 petals for 08:00–21:00)
  - Sized by average revenue that hour across all stores
  - Color intensity by conversion rate (transactions/traffic)
  - Shows peak revenue windows

- **Treemap (middle-right, 320px):**
  - Revenue breakdown: Store Format → Individual Store
  - Sized by total revenue
  - Color by growth rate (green = growing, red = declining)

- **Histogram (bottom-left, 280px):**
  - Basket size distribution (bins: $0–$25, $25–$50, $50–$100, $100–$150, $150+)
  - Derived from hourly_metrics.avg_basket
  - Overlay: line showing cumulative % of transactions

- **Bubble chart (bottom-right, 280px):**
  - X: Store sqft, Y: Revenue/sqft, Bubble size: Employee count
  - Color by format
  - Helps identify over/understaffed locations relative to revenue density
