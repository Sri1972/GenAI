...
### P0.3: User & Group Member Management (Full CRUD)

**User Story:**  
As Priya, I want to manage a roster of people (add, edit, delete) so that I can quickly assign them to expense splits without re-entering details each time.

**Acceptance Criteria:**

- [ ] **Users Tab:** Dedicated "People" or "Users" tab in the main navigation for managing the user roster.
- [ ] **Add User:** User can add a new person with: full name (required), email (required, unique), phone (optional), avatar/initials (auto-generated from name).
- [ ] **Edit User:** User can update any person's name, email, or phone from the roster list or a detail view.
- [ ] **Delete User:** User can delete a person from the roster (with confirmation dialog). Deletion is blocked if the person has unsettled splits (warning shown).
- [ ] **User List:** Roster displays all added people in a searchable, sortable table/card view with columns: Name, Email, Phone, # of Splits, Total Owed/Owes, Status (active/inactive).
- [ ] App displays list of added members with their assigned items and amounts within a split.
- [ ] User can add/remove members from a split before settlement is finalized (picks from existing roster or creates inline).
- [ ] Each member has a unique identifier (email).
...
- Users can manage their roster independently of any specific split.
...
### P0.8: Multi-Person Expense Splitting

**User Story:**  
As Sarah, I want to select a set of expenses and split them among multiple people (equally or by custom amounts/percentages) so that everyone pays their fair share.

**Acceptance Criteria:**

- [ ] **New Split Flow:** User clicks "New Split" → selects people from roster (multi-select) → adds expenses/line items → chooses split method.
- [ ] **Split Methods:**
  - Equal split: Total divided equally among selected people.
  - Percentage split: User assigns custom percentages per person (must total 100%).
  - Custom amounts: User enters exact amount per person (must total expense amount).
  - By item: Individual line items assigned to specific people (existing OCR flow).
- [ ] **Expense Entry:** User can add expenses manually (description, amount, date, category) or via receipt OCR upload.
- [ ] **Multiple Expenses per Split:** A single split session can include multiple expenses (e.g., dinner + taxi + hotel for a trip).
- [ ] **Split Summary:** Before confirmation, app shows clear breakdown: each person's total, payment method, and settlement instructions.
- [ ] **Settlement Optimization:** App calculates minimum number of transactions needed to settle all debts within the split.
- [ ] Split is saved and visible in history/dashboard for all participants.

**Success Metric:**  
- 90%+ of splits successfully assign all amounts to participants.
- Average time to create a 4-person split <3 minutes.
- Zero mathematical errors in split calculations.

---

### P0.9: Overall Balances (Per-Person Net Position)

**User Story:**  
As Sarah, I want to see at a glance how much each person owes me (and I owe them) across ALL my splits, so I know exactly where I stand financially with everyone.

**Acceptance Criteria:**

- [ ] **Balances Page/Tab:** Dedicated "Balances" section accessible from main navigation (this is the primary home screen).
- [ ] **Per-Person Net View:** Shows each person in the user's roster with:
  - Net balance (positive = they owe you, negative = you owe them).
  - Visual indicator (green for "owes you", red for "you owe").
  - Number of active/unsettled splits contributing to the balance.
- [ ] **Net Calculation:** Balance is computed across ALL open splits with that person:
  - If you owe Alice $30 from dinner and Alice owes you $50 from trip → net: Alice owes you $20.
- [ ] **Quick Actions:** From balance view, user can:
  - "Settle Up" → triggers settle-up flow (see P1.8).
  - "View Splits" → shows all active splits with that person.
  - "Send Reminder" → sends payment reminder email.
- [ ] **Summary Header:** Top of page shows aggregate: "You are owed $X total" / "You owe $Y total" / "Net position: +$Z".
- [ ] **Sort/Filter:** Sort by amount (highest first), by name, or by last activity. Filter: "owes me" / "I owe" / "settled".
- [ ] **Zero-balance:** People with $0 net balance shown separately or hidden (toggle).
- [ ] Balance updates in real-time when splits are created, modified, or settled.

**Success Metric:**  
- Balances page is the most-visited page in the app (>50% of sessions start here).
- Net balance calculation is 100% accurate across all edge cases.
- Users can identify who to follow up with in <5 seconds.

**Out of Scope:**  
- Multi-currency conversion (MVP: single currency).
- Automatic bank transfer to settle (external payment only).
...
### P1.7: Analytics Dashboard (Dedicated Tab)

**User Story:**  
As Priya, I want a dedicated Analytics tab that shows spending trends, splitting patterns, and balances so that I can understand my expense-sharing behavior at a glance.

**Acceptance Criteria:**

- [ ] **Analytics Tab:** Separate tab/page in main navigation labeled "Analytics" (accessible from sidebar/nav).
- [ ] **Summary KPIs (top row):**
  - Total amount split (all time & this month).
  - Total you owe (outstanding balance across all splits).
  - Total owed to you (outstanding balance others owe you).
  - Number of active splits.
- [ ] **Spending Over Time Chart:** Line or bar chart showing monthly total expenses split, filterable by date range (last 30 days, 90 days, 6 months, 1 year).
- [ ] **Category Breakdown:** Pie/donut chart showing expense distribution by category (Food, Travel, Rent, Utilities, Other).
- [ ] **Top Splitting Partners:** Ranked list or bar chart of people you split with most frequently, with total amounts.
- [ ] **Settlement Status:** Stacked bar or progress chart showing paid vs. unpaid across all active splits.
- [ ] **Balance History:** Line chart showing your net balance over time (positive = owed to you, negative = you owe).
- [ ] **Filters:** User can filter analytics by date range, group, category, or specific person.
- [ ] **Responsive:** Charts render correctly on mobile (stacked layout) and desktop (grid layout).
- [ ] **Data is real-time:** Analytics reflect current state of all splits and settlements.

**Success Metric:**  
- 60%+ of active users visit Analytics tab at least once per week.
- Dashboard loads in <2 seconds.
- Users report improved visibility into spending patterns (NPS survey).

**Out of Scope:**  
- Predictive analytics or ML-based insights (future enhancement).
- Export analytics as PDF/image (P2 feature).
- Budget setting or spending alerts (future feature).

---

### P1.8: Settle Up (Per-Person Settlement)

**User Story:**  
As Marcus, I want to "Settle Up" with a specific person for the net amount across all our splits, rather than marking each split individually, so that settling debts is quick and simple.

**Acceptance Criteria:**

- [ ] **Settle Up Action:** Available from the Balances page next to each person with a non-zero balance.
- [ ] **Settlement Summary:** Shows:
  - Net amount owed (computed across all open splits with that person).
  - Breakdown: list of contributing splits (name, amount, date).
  - "Record Full Payment" button (records net amount as settled).
  - "Record Partial Payment" option (enter custom amount; remaining balance stays open).
- [ ] **Payment Recording:** When user records a settlement:
  - Marks contributing splits as settled/partially settled (oldest first, or proportional).
  - Stores payment method (Venmo, PayPal, cash, bank transfer) and optional note.
  - Timestamps the settlement.
  - Sends confirmation email to both parties.
  - Updates the Balances page immediately.
- [ ] **Settlement History:** Each person's detail view shows a log of past settlements (date, amount, method, note).
- [ ] **Undo:** User can undo a settlement within 24 hours (reverts split statuses).

**Success Metric:**  
- 70%+ of settlements use "Settle Up" rather than marking individual splits.
- Average time to settle up <30 seconds.
- Zero errors in net balance computation after settlement.

**Out of Scope:**  
- Direct payment processing (Venmo/PayPal integration — P2).
- Automatic payment detection (future enhancement).

---

### P1.9: Activity Feed & In-App Notifications

**User Story:**  
As Priya, I want to see a feed of recent activity (new splits, payments received, reminders) inside the app so I don't have to rely on email to stay informed.

**Acceptance Criteria:**

- [ ] **Activity Feed:** Accessible from main navigation (bell icon + "Activity" tab or sidebar section).
- [ ] **Event Types Displayed:**
  - "Sarah added you to split 'Dinner at Mario's' — you owe $25.50"
  - "Bob paid you $30.00 for 'Weekend Trip'"
  - "Payment reminder: You owe Alice $15.00 (due in 2 days)"
  - "Recurring split 'Rent - March' was generated — you owe $800"
  - "Sarah edited split 'Dinner at Mario's' — your amount changed to $27.00"
- [ ] **Unread Count:** Badge on notification icon showing number of unread notifications.
- [ ] **Mark as Read:** User can mark individual notifications as read, or "Mark all as read".
- [ ] **Clickable:** Each notification links to the relevant split/settlement detail page.
- [ ] **Chronological:** Feed shows most recent first, with date grouping (Today, Yesterday, This Week, Older).
- [ ] **Persistence:** Notifications persist for 90 days; older ones are archived/deleted.
- [ ] **Real-time (stretch):** New notifications appear without page refresh (WebSocket or polling).

**Success Metric:**  
- 50%+ of users check activity feed at least 3x per week.
- Notification delivery latency <30 seconds after triggering event.
- Unread badge drives 2x higher engagement vs. email-only.

**Out of Scope:**  
- Push notifications (browser push / mobile — P2/PWA feature).
- Notification preferences/muting specific people (P2.9 covers this).
- In-app messaging between users (P2.3).
...
### P2.5: ~~Analytics & Insights~~ — **PROMOTED TO P1.7**

**Note:** This feature has been elevated to P1 priority. See P1.7 below.
