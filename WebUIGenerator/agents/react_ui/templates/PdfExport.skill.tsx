...
export default function PdfExportPage() {
  ...
  const generate = () => {
    ...
    setDone(false)
    try {
      // Estimate widest section to decide orientation
      const avgCharWidth = 2.1 // mm per character at fontSize 8.5
      const cellPadding = 6
      const margins = 28
      const widestSection = Math.max(...(sections as any[]).map((sec: any) => {
        const cols = (sec.columns as any[]) ?? []
        const rows = getRows(sec)
        return cols.reduce((sum: number, col: any) => {
          const headerLen = (col.header || '').length
          const maxDataLen = rows.slice(0, 20).reduce((mx: number, row: any) => {
      ...
      const orientation = widestSection > 182 ? 'landscape' : 'portrait'
