...
# PPT server parameters for PowerPoint generation
ppt_server_path = Path(__file__).resolve().parents[2] / 'server' / 'ppt' / 'ppt_mcp_server.py'

ppt_server_params = StdioServerParameters(
    command=sys.executable,
    args=[str(ppt_server_path)]
)
...
async def run(query: str, chat_id: str = "default"):
    try:
        ...
        print("=" * 60)
        print(f"[1/6] Connecting to PMO MCP server...")
        ...
        async with stdio_client(server_params) as (pmo_reader, pmo_writer):
            async with ClientSession(pmo_reader, pmo_writer) as pmo_session:
                ...
                print(f"[2/6] ✓ PMO MCP server initialized successfully")
                
                # Connect to Chart MCP server
                print(f"[3/6] Attempting to connect to Chart MCP server...")
                ...
                try:
                    async with stdio_client(chart_server_params) as (chart_reader, chart_writer):
                        async with ClientSession(chart_reader, chart_writer) as chart_session:
                            ...
                            print(f"[4/6] ✓ Chart MCP server initialized successfully")
                            
                            # Connect to PPT MCP server
                            print(f"[5/6] Attempting to connect to PowerPoint MCP server...")
                            print(f"       Server path: {ppt_server_path}")
                            ppt_session = None
                            try:
                                async with stdio_client(ppt_server_params) as (ppt_reader, ppt_writer):
                                    async with ClientSession(ppt_reader, ppt_writer) as ppt_session:
                                        await ppt_session.initialize()
                                        print(f"[6/6] ✓ PowerPoint MCP server initialized successfully")
                                        print(f"       All 3 servers ready - proceeding to tool discovery...")
                                        print("=" * 60)
                                        
                                        # Continue with main logic using all three sessions
                                        return await run_with_sessions(query, chat_id, pmo_session, chart_session, ppt_session)
                            except Exception as ppt_err:
                                print(f"[6/6] ⚠ PowerPoint server not available: {ppt_err}")
                                print(f"       Continuing with PMO and Chart servers (PPT generation disabled)...")
                                print("=" * 60)
                                return await run_with_sessions(query, chat_id, pmo_session, chart_session, None)
                except Exception as chart_err:
                    print(f"[4/6] ⚠ Chart server not available: {chart_err}")
                    print(f"[5/6] Skipping PowerPoint server (requires Chart server)...")
                    print(f"[6/6] Continuing with PMO server only (chart and PPT generation disabled)...")
                    print("=" * 60)
                    return await run_with_sessions(query, chat_id, pmo_session, None, None)
    except Exception as e:
...
async def run_with_sessions(query: str, chat_id: str, pmo_session: ClientSession, chart_session: ClientSession | None, ppt_session: ClientSession | None):
    """Main logic with access to PMO, Chart, and PowerPoint MCP sessions."""
    try:
        ...
        # Add PPT tools if available
        ppt_tools_count = 0
        if ppt_session:
            print(f"[TOOL DISCOVERY] Loading PowerPoint server tools...")
            ppt_tools_result = await ppt_session.list_tools()
            ppt_tools_count = len(ppt_tools_result.tools) if hasattr(ppt_tools_result, 'tools') else 0
            print(f"[TOOL DISCOVERY] ✓ Loaded {ppt_tools_count} PowerPoint tools")
            print(f"Available PowerPoint tools: {ppt_tools_count} tools loaded")
        ...
        # Add PPT tools if ppt session is available
        if ppt_session:
            print("Adding PowerPoint tools to available tools list...")
        ...
        tool_descriptions = "\n".join(tool_lines)
        print(f"Total tools available: {len(tool_lines)} ({tool_count} PMO + {chart_tools_count} Chart + {ppt_tools_count} PPT)")
        ...
        # Add PowerPoint generation instructions if PPT server is available
        if ppt_session:
            ppt_instructions = (
                "\n- POWERPOINT GENERATION:\n"
                "  * **ONLY create PowerPoint presentations when user EXPLICITLY requests** with keywords: 'create ppt', 'create powerpoint', 'export to ppt', 'make presentation', 'generate slides'\n"
                "  * **WORKFLOW**: PPT generation requires PNG chart images from the Chart server first:\n"
                "    1. Generate charts using render_chart_from_dataset tool (creates HTML + PNG automatically)\n"
                "    2. Extract PNG file paths from chart generation output (look for .png files in D:\\SourceCode\\GenAI\\MCP\\client\\pmo\\html-charts\\)\n"
                "    3. Create presentation using create_presentation tool\n"
                "    4. Add title slide with add_title_slide\n"
                "    5. Add content slides with bullet points using add_content_slide\n"
                "    6. Add chart slides with PNG images using add_image_slide (provide PNG path from step 2)\n"
                "    7. Add analysis/summary slides using add_content_slide or add_two_column_slide\n"
                "    8. Add tables using add_table_slide for detailed data\n"
                "    9. Save presentation using save_presentation to D:\\SourceCode\\GenAI\\MCP\\client\\pmo\\presentations\\\n"
                "  * **PRESENTATION STRUCTURE** (Professional PMO presentations should include):\n"
                "    - Title slide with presentation name and date\n"
                "    - Executive Summary slide with key metrics (use add_content_slide)\n"
                "    - Detailed data slides with charts (use add_image_slide with PNG from chart server)\n"
                "    - Analysis slides with insights and bullet points\n"
                "    - Tables for detailed breakdowns (use add_table_slide)\n"
                "    - Summary/conclusion slide with recommendations\n"
                "  * **PPT TOOL USAGE**:\n"
                "    - create_presentation: presentation_id (string)\n"
                "    - add_title_slide: presentation_id, title, subtitle (optional)\n"
                "    - add_content_slide: presentation_id, title, content (array of bullet points)\n"
                "      * Optional styling: font_size (int, e.g., 18, 24), font_color (RGB array [R,G,B] e.g., [0,0,0] for black, [255,0,0] for red)\n"
                "      * Optional title styling: title_font_size (int), title_font_color (RGB array)\n"
                "      * Optional borders: border_color (RGB array), border_width (float, e.g., 1.5, 2.0)\n"
                "    - add_two_column_slide: presentation_id, title, left_content (array), right_content (array)\n"
                "      * Same optional styling parameters as add_content_slide\n"
                "    - add_image_slide: presentation_id, title, image_path (PNG from chart server)\n"
                "    - add_chart_slide: presentation_id, title, chart_type, categories, series_data (for native PPT charts)\n"
                "    - add_table_slide: presentation_id, title, headers (array), rows (array of arrays)\n"
                "    - save_presentation: presentation_id, output_path\n"
                "  * **STYLING EXAMPLES**:\n"
                ...
                "    - RGB common colors: Black=[0,0,0], White=[255,255,255], Red=[255,0,0], Blue=[0,0,255], Green=[0,128,0]\n"
                "  * **EXAMPLE PNG PATH**: D:\\SourceCode\\GenAI\\MCP\\client\\pmo\\html-charts\\project_costs_bar_d3_20251124_143022.png\n"
                "  * **MULTI-STEP PLAN**: PPT generation requires multiple steps - create a plan with steps for:\n"
                "    1. Data fetching (get_all_projects, etc.)\n"
                "    2. Chart generation (render_chart_from_dataset) - note the PNG paths\n"
                "    3. PPT creation (create_presentation)\n"
                "    4. Adding slides (add_title_slide, add_image_slide, add_content_slide, etc.)\n"
                "    5. Saving (save_presentation)\n"
            )
        else:
            ppt_instructions = (
                "\n- POWERPOINT GENERATION:\n"
                "  * **PPT TOOLS NOT AVAILABLE** - The PowerPoint MCP server is not connected to this client\n"
                "  * When user requests PowerPoint presentations, inform them that PPT generation is currently unavailable\n"
                "  * You can provide the data in markdown or table format instead\n"
            )
        ...
        while iteration < max_iterations:
            ...
            if parsed and isinstance(parsed, dict) and 'plan' in parsed and isinstance(parsed.get('plan'), list):
                ...
                for idx, step in enumerate(steps):
                    try:
                        ...
                        # Determine routing - check PPT tools first, then chart tools
                        is_ppt_tool = tool_name in ['create_presentation', 'add_title_slide', 'add_content_slide', 
                                                     'add_two_column_slide', 'add_chart_slide', 'add_table_slide',
                                                     'add_image_slide', 'add_gantt_chart_slide', 'add_process_flow_slide',
                                                     'add_timeline_slide', 'add_diagram_slide', 'add_shape_slide',
                                                     'save_presentation', 'list_presentations']
                        is_chart_tool = tool_name.startswith('render_chart') or (tool_name.startswith('create_') and not is_ppt_tool)

                        print(f"\n[PLAN STEP {idx+1}/{len(steps)}] Executing {tool_name}")
                        if is_ppt_tool:
                            if ppt_session:
                                print(f"[ROUTING] Using PowerPoint MCP server")
                            else:
                                print(f"[ERROR] PowerPoint tool requested but PPT server unavailable")
                                plan_results[step.get('id', f'step_{idx+1}')] = json.dumps({"error": "PowerPoint server not available"})
                                continue
                        elif is_chart_tool:
                            if chart_session:
                        ...
                        try:
                            ...
                            if is_ppt_tool and ppt_session:
                                tool_result = await ppt_session.call_tool(tool_name, tool_args)
                            elif is_chart_tool and chart_session:
            ...
            if parsed and isinstance(parsed, dict) and 'tool' in parsed:
                ...
                # Determine which server to route to - check PPT tools first, then chart tools
                is_ppt_tool = tool_name in ['create_presentation', 'add_title_slide', 'add_content_slide', 
                                             'add_two_column_slide', 'add_chart_slide', 'add_table_slide',
                                             'add_image_slide', 'add_gantt_chart_slide', 'add_process_flow_slide',
                                             'add_timeline_slide', 'add_diagram_slide', 'add_shape_slide',
                ...
                if is_ppt_tool:
                    if ppt_session:
                        print(f"\n[TOOL CALL] Routing to PowerPoint MCP server: {tool_name}")
                    else:
                        print(f"\n[ERROR] PowerPoint tool requested but PPT server unavailable: {tool_name}")
                        conversation_messages.append({"role": "user", "content": f"[ERROR] PowerPoint tool '{tool_name}' requested but PPT server is not connected. PowerPoint generation is unavailable."})
                        continue
                elif is_chart_tool:
                    if chart_session:
