...
def _enforce_rules(files: dict) -> dict:
    from agents.figma_to_web_using_api_agent import _enforce_rules as _real_enforce
    return _real_enforce(files)
...
def start_figma_project(project_name: str) -> dict:
    raise RuntimeError(
        f"HTML project serving is no longer supported. "
        f"Project '{project_name}' should be generated as a React/Vite app instead."
    )
...
def generate_from_figma(
    ...
    # ── Step 4: Return project info (no HTML server — React pipeline preferred) ─
    _p("ready")
