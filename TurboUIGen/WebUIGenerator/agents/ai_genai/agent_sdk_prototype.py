"""
AI/GenAI Engineer Agent — SDK prototype.

Handles: AI-powered page generation (Stage 5) — chat interfaces, copilots,
data chat, NLQ features.

Tools: read_skill_template (for AiChat/DataChat templates), validate_react_component
"""
...
def _build_system_prompt(stage: str = "pages") -> str:
    ...
    parts.append("\nFor AI CHAT pages, use the read_skill_template tool with 'AiChat' or "
                 "'DataChat' as a starting reference, then customize "
                 "for the specific app's needs.")
...
def run_ai_genai(
    ...
    """
    Run the AI/GenAI agent.

    Used by orchestrator for:
      - Stage 5 (pages): generates pages with type ai-chat, data-chat, copilot, assistant

    Replaces: get_agent("ai_genai").generate(prompt, context=..., stage=..., json_mode=True, ...)
    """
    messages = []
