...
## 5.4 User Roster Management Endpoints (CRUD)

### GET /users
**Purpose:** List all users in the current user's roster (people available for splitting)  
**Authentication:** Required (Bearer token)  
**Rate Limit:** 100 req/min per user

**Query Parameters:**
```
?limit=50&offset=0&sort=fullName&order=asc&search=john
```
...
### POST /users
**Purpose:** Add a new person to the user's roster  
**Authentication:** Required (Bearer token)  
...
- `email`: Required, valid email format, unique within roster
- `phone`: Optional, valid phone format (E.164 or common formats)
...
| Status | Code | Message |
|--------|------|---------|
| 400 | `VALIDATION_ERROR` | Full name is required / Invalid email format |
| 409 | `DUPLICATE_EMAIL` | A user with this email already exists in your roster |
| 401 | `UNAUTHORIZED` | Missing or invalid authentication token |
...
**Request Validation:**
- `fullName`: Optional, min 2 characters, max 100 characters
- `email`: Optional, valid email format, unique within roster
- `phone`: Optional, valid phone format or null to remove
...
| Status | Code | Message |
|--------|------|---------|
| 400 | `VALIDATION_ERROR` | Invalid email format / Name too short |
| 404 | `USER_NOT_FOUND` | User not found in your roster |
| 409 | `DUPLICATE_EMAIL` | A user with this email already exists in your roster |

---

### DELETE /users/:userId
**Purpose:** Remove a person from the roster  
**Authentication:** Required (Bearer token)  
**Rate Limit:** 50 req/min per user
...
|--------|------|---------|
| 404 | `USER_NOT_FOUND` | User not found in your roster |
| 409 | `HAS_UNSETTLED_SPLITS` | Cannot delete user with unsettled splits. Settle outstanding balances first. |

**Business Rules:**
- Cannot delete a user who has unsettled (open/partially_settled) splits
- Deletion is soft-delete (status → 'deleted'); historical split records preserved
- Deleted users no longer appear in roster list or split member picker

---

## 5.5 Expense Splitting Endpoints

### POST /splits
**Purpose:** Create a new expense split among selected people  
**Authentication:** Required (Bearer token)  
**Rate Limit:** 50 req/min per user
...
```json
{
  "description": "Dinner at Mario's - March 15",
  "groupId": "grp_abc123",
  "expenses": [
    {
  ...
  "participants": ["usr_abc123", "usr_xyz789", "usr_def456"],
  "splitMethod": "equal",
  "splitDetails": null
}
```
...
- `"percentage"` — `splitDetails`: `{"usr_abc123": 40, "usr_xyz789": 30, "usr_def456": 30}` (must sum to 100)
- `"custom"` — `splitDetails`: `{"usr_abc123": 4000, "usr_xyz789": 3048, "usr_def456": 3000}` (amounts in cents, must sum to total)
- `"by_item"` — `splitDetails`: per-item assignment (used with OCR flow)
...
```json
{
  "split": {
    ...
    "participants": [
      {"userId": "usr_abc123", "fullName": "John Doe", "amount": 3350, "status": "pending"},
      {"userId": "usr_xyz789", "fullName": "Jane Smith", "amount": 3349, "status": "pending"},
      {"userId": "usr_def456", "fullName": "Bob Wilson", "amount": 3349, "status": "pending"}
    ],
    "settlements": [
      {"fromUserId": "usr_xyz789", "toUserId": "usr_abc123", "amount": 3349},
      {"fromUserId": "usr_def456", "toUserId": "usr_abc123", "amount": 3349}
    ]
  }
}
```
...
| Status | Code | Message |
|--------|------|---------|
| 400 | `VALIDATION_ERROR` | At least 2 participants required / Percentages must sum to 100 / Amounts must sum to total |
| 404 | `USER_NOT_FOUND` | One or more participants not found |

---

### GET /splits
**Purpose:** List all splits for the authenticated user  
**Authentication:** Required (Bearer token)

**Query Parameters:**
```
?limit=20&offset=0&status=open&sort=createdAt&order=desc
```
...
### PUT /splits/:splitId/settle/:userId
**Purpose:** Mark a participant's payment as settled  
**Authentication:** Required (Bearer token)
...
**Purpose:** Get aggregate KPIs for the authenticated user  
**Authentication:** Required (Bearer token)
...
```json
{
  "totalSplitAmount": 245000,
  "totalSplitAmountThisMonth": 52000,
  "totalYouOwe": 12500,
  "totalOwedToYou": 34200,
  "activeSplitsCount": 5,
  "settledSplitsCount": 37,
  "averageSplitAmount": 5833
}
```

---

### GET /analytics/spending-over-time
**Purpose:** Get monthly spending totals for charting  
**Authentication:** Required (Bearer token)

**Query Parameters:**
```
?period=monthly&dateFrom=2023-10-01&dateTo=2024-03-31
```
...
{
  "data": [
    {"period": "2023-10", "amount": 32000, "splitCount": 4},
    {"period": "2023-11", "amount": 41500, "splitCount": 6},
    {"period": "2023-12", "amount": 58000, "splitCount": 8},
    {"period": "2024-01", "amount": 29000, "splitCount": 3},
    {"period": "2024-02", "amount": 45000, "splitCount": 5},
    {"period": "2024-03", "amount": 52000, "splitCount": 7}
  ]
}
```
...
{
  "data": [
    {"category": "food", "amount": 98000, "percentage": 40, "count": 15},
    {"category": "travel", "amount": 62000, "percentage": 25, "count": 4},
    {"category": "rent", "amount": 50000, "percentage": 20, "count": 3},
    {"category": "utilities", "amount": 25000, "percentage": 10, "count": 6},
    {"category": "other", "amount": 10000, "percentage": 5, "count": 2}
  ]
}
```
...
**Purpose:** Get most frequent splitting partners  
**Authentication:** Required (Bearer token)

**Query Parameters:**
```
?limit=10&dateFrom=2024-01-01&dateTo=2024-03-31
```
...
{
  "data": [
    {"userId": "usr_xyz789", "fullName": "Jane Smith", "splitCount": 12, "totalAmount": 85000},
    {"userId": "usr_def456", "fullName": "Bob Wilson", "splitCount": 8, "totalAmount": 52000},
    {"userId": "usr_ghi012", "fullName": "Alice Chen", "splitCount": 5, "totalAmount": 31000}
  ]
}
```
...
**Purpose:** Get net balance over time (positive = owed to you, negative = you owe)  
**Authentication:** Required (Bearer token)
...
{
  "data": [
    {"period": "2023-10", "balance": 5200},
    {"period": "2023-11", "balance": -3100},
    {"period": "2023-12", "balance": 8400},
    {"period": "2024-01", "balance": 12500},
    {"period": "2024-02", "balance": 9800},
    {"period": "2024-03", "balance": 21700}
  ]
}
```
...
## 5.8 Balances & Settle-Up Endpoints

### GET /balances
**Purpose:** Get per-person net balances across all splits for the authenticated user  
**Authentication:** Required (Bearer token)

**Query Parameters:**
```
?filter=owes_me|i_owe|all&sort=amount|name|lastActivity&order=desc
```
  ...
  "balances": [
    {
      ...
      "direction": "owes_you",
      "activeSplitCount": 3,
      "lastActivityAt": "2024-03-18T14:30:00Z",
      "splits": [
        {"splitId": "spl_001", "description": "Dinner at Mario's", "amount": 5000, "date": "2024-03-15"},
        {"splitId": "spl_007", "description": "Weekend Trip", "amount": 10000, "date": "2024-03-10"}
      ]
    },
    {
      "userId": "usr_def456",
      ...
      "direction": "you_owe",
      "activeSplitCount": 2,
      "lastActivityAt": "2024-03-16T09:00:00Z",
      "splits": [
        {"splitId": "spl_003", "description": "Groceries", "amount": -3500, "date": "2024-03-16"},
        {"splitId": "spl_005", "description": "Internet Bill", "amount": -5000, "date": "2024-03-01"}
      ]
    }
  ]
}
...
**Business Rules:**
- Net balance computed as: sum(amounts others owe you in all open splits with that person) minus sum(amounts you owe them)
- Positive = they owe you; Negative = you owe them
- Only includes open/partially_settled splits (settled splits excluded)
- People with $0 net balance included but can be filtered via query param

---

### POST /balances/:userId/settle
**Purpose:** Record a settlement (full or partial) with a specific person  
**Authentication:** Required (Bearer token)

**Path Parameters:**
- `userId`: Person being settled up with
...
```json
{
  ...
  "type": "full",
  "paymentMethod": "venmo",
  "notes": "Sent via Venmo 3/18"
}
```

**Request Validation:**
- `amount`: Required, positive integer (cents), must not exceed net balance
- `type`: `"full"` (settles entire net balance) or `"partial"` (settles specified amount)
- `paymentMethod`: Optional, one of: `venmo`, `paypal`, `cash`, `bank_transfer`, `other`
...
```json
{
  "settlement": {
    ...
    "paymentMethod": "venmo",
    "notes": "Sent via Venmo 3/18",
    "settledAt": "2024-03-18T15:00:00Z",
    "affectedSplits": [
      {"splitId": "spl_001", "newStatus": "settled", "settledAmount": 5000},
      {"splitId": "spl_007", "newStatus": "settled", "settledAmount": 10000}
    ]
  },
  "newNetBalance": 0
}
...
| Status | Code | Message |
|--------|------|---------|
| 400 | `INVALID_AMOUNT` | Amount exceeds net balance / Amount must be positive |
| 404 | `USER_NOT_FOUND` | User not found or no active balance |
| 409 | `ALREADY_SETTLED` | No outstanding balance with this person |

**Business Rules:**
- Full settlement: marks all contributing splits with that person as settled
- Partial settlement: applies amount to oldest splits first (FIFO)
- Settlement is timestamped and linked to affected splits
- Both parties' balance views update immediately
- Email notification sent to the other party confirming settlement

---

### GET /balances/:userId/history
**Purpose:** Get settlement history with a specific person  
**Authentication:** Required (Bearer token)
...
## 5.9 Activity Feed & Notifications Endpoints

### GET /notifications
**Purpose:** Get the authenticated user's activity feed/notifications  
**Authentication:** Required (Bearer token)
  ...
  "notifications": [
    {
      ...
      "type": "split_created",
      "title": "New split",
      "message": "Sarah added you to 'Dinner at Mario's' — you owe $25.50",
      "relatedSplitId": "spl_001",
      "relatedUserId": "usr_sarah",
      "isRead": false,
      "createdAt": "2024-03-18T22:15:00Z"
    },
    {
      "id": "ntf_002",
      "type": "payment_received",
      "title": "Payment received",
      "message": "Bob paid you $30.00 for 'Weekend Trip'",
      "relatedSplitId": "spl_007",
      "relatedUserId": "usr_bob",
      "isRead": true,
...
**Notification Types:**
- `split_created` — User was added to a new split
- `split_updated` — A split the user is in was modified (amount changed)
- `payment_received` — Someone settled/paid the user
- `payment_reminder` — Upcoming due date reminder
- `payment_overdue` — Past-due payment reminder
- `recurring_generated` — A recurring split instance was created
- `settlement_recorded` — A settle-up was recorded involving the user
...
## 5.10 Receipt & OCR Processing Endpoints
...
│   │   │       └── user.module.css
│   │   │
│   │   ├── people/                    # User roster management (CRUD)
│   │   │   ├── PeopleListPage.tsx     # Main roster page with searchable table of all people
│   │   │   ├── PersonCard.tsx         # Compact card showing person name, email, split stats
│   │   │   ├── PersonForm.tsx         # Add/Edit person form (name, email, phone)
│   │   │   ├── PersonDetailPage.tsx   # Detail view: person info, their splits, balances
│   │   │   ├── DeletePersonDialog.tsx # Confirmation dialog for deleting a person
│   │   │   └── styles/
│   │   │       └── people.module.css
│   │   │
│   │   ├── balances/                  # Overall balances (per-person net position)
│   │   │   ├── BalancesPage.tsx       # Main balances page — primary home screen
│   │   │   ├── BalanceSummaryHeader.tsx # Top summary: total owed to you / you owe / net
│   │   │   ├── PersonBalanceCard.tsx  # Card per person: name, net amount, direction, action buttons
│   │   │   ├── SettleUpModal.tsx      # Modal: settlement summary, full/partial, payment method
│   │   │   ├── SettlementHistory.tsx  # List of past settlements with a person
│   │   │   └── styles/
│   │   │       └── balances.module.css
│   │   │
│   │   ├── activity/                  # Activity feed & in-app notifications
│   │   │   ├── ActivityFeedPage.tsx   # Full activity feed page (chronological)
│   │   │   ├── NotificationItem.tsx   # Single notification row (icon, message, time, link)
│   │   │   ├── NotificationBell.tsx   # Bell icon with unread badge (used in header/nav)
│   │   │   ├── NotificationDropdown.tsx # Quick-view dropdown from bell (last 5 + "View All")
│   │   │   └── styles/
│   │   │       └── activity.module.css
│   │   │
│   │   ├── analytics/                 # Analytics dashboard (dedicated tab)
│   │   │   ├── AnalyticsPage.tsx      # Main analytics page with KPIs and charts
│   │   │   ├── KpiSummaryRow.tsx      # Top row of KPI cards (total split, owed, owes, active)
│   │   │   ├── SpendingChart.tsx      # Line/bar chart of monthly spending over time
│   │   │   ├── CategoryPieChart.tsx   # Donut/pie chart of expense categories
│   │   │   ├── TopPartnersChart.tsx   # Horizontal bar chart of top splitting partners
│   │   │   ├── BalanceHistoryChart.tsx # Line chart of net balance over time
│   │   │   ├── SettlementStatusChart.tsx # Stacked bar of paid vs unpaid across splits
│   │   │   ├── AnalyticsFilters.tsx   # Date range, category, and person filter controls
│   │   │   └── styles/
...
│   │   ├── usePeople.ts              # Hook for user roster CRUD (add, edit, delete, list)
│   │   ├── useBalances.ts            # Hook for per-person net balances and settle-up actions
│   │   ├── useNotifications.ts       # Hook for activity feed, unread count, mark-as-read
│   │   ├── useAnalytics.ts           # Hook for analytics data fetching (KPIs, charts)
...
export interface RosterPerson {
  ...
  splitCount: number;
  totalOwed: number;             // Amount others owe this person (cents)
  totalOwes: number;             // Amount this person owes others (cents)
  status: 'active' | 'inactive' | 'deleted';
  createdAt: string;
  updatedAt: string;
}

// Multi-Person Expense Split
export interface ExpenseSplit {
  id: string;
  description: string;
  totalAmount: number;           // In cents
  splitMethod: 'equal' | 'percentage' | 'custom' | 'by_item';
...
export interface SplitParticipant {
  userId: string;
  fullName: string;
  amount: number;                // Amount this person owes (cents)
...
export interface AnalyticsSummary {
  ...
  totalSplitAmountThisMonth: number;
  totalYouOwe: number;
  totalOwedToYou: number;
  activeSplitsCount: number;
  settledSplitsCount: number;
...
export interface BalanceSummary {
  ...
  netPosition: number;             // Cents (positive = net positive)
}
...
export interface PersonBalance {
  ...
  splits: BalanceSplitEntry[];
}

export interface BalanceSplitEntry {
  splitId: string;
  description: string;
  amount: number;                  // Positive = owed to you from this split
  date: string;
}
...
export interface Settlement {
  ...
  amount: number;
  type: 'full' | 'partial';
  paymentMethod?: 'venmo' | 'paypal' | 'cash' | 'bank_transfer' | 'other';
  notes?: string;
  settledAt: string;
  affectedSplits: { splitId: string; newStatus: string; settledAmount: number }[];
}

// Activity Feed / Notifications
export interface Notification {
  id: string;
  type: 'split_created' | 'split_updated' | 'payment_received' | 'payment_reminder' | 'payment_overdue' | 'recurring_generated' | 'settlement_recorded';
  title: string;
  message: string;
  relatedSplitId?: string;
...
## 6.4 Balances Page Specification (Primary Home Screen)

### BalancesPage (src/components/balances/BalancesPage.tsx)

**Purpose:** The main landing page — shows per-person net balances at a glance across all splits.

**Layout:**
```
┌─────────────────────────────────────────────────────────┐
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
│  │ Owed to You  │  │  You Owe     │  │ Net Position │  │
│  │   $342.00    │  │   $125.00    │  │  +$217.00    │  │
│  └──────────────┘  └──────────────┘  └──────────────┘  │
├─────────────────────────────────────────────────────────┤
│  [Filter: All ▾]  [Sort: Amount ▾]                      │
├─────────────────────────────────────────────────────────┤
│  ┌─────────────────────────────────────────────────┐    │
│  │ JS  Jane Smith          +$150.00  [Settle Up]  │    │
│  │     3 active splits · Last: Mar 18             │    │
│  ├─────────────────────────────────────────────────┤    │
│  │ BW  Bob Wilson           -$85.00  [Remind]     │    │
│  │     2 active splits · Last: Mar 16             │    │
│  ├─────────────────────────────────────────────────┤    │
│  │ AC  Alice Chen           +$42.00  [Settle Up]  │    │
│  │     1 active split  · Last: Mar 12             │    │
│  └─────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────┘
```

**Interactions:**
- Click "Settle Up" → opens SettleUpModal for that person
- Click person row → navigates to split list filtered by that person
- Click "Remind" → sends payment reminder email to that person
- Filter: "Owes Me" / "I Owe" / "All"
- Sort: by amount (highest first), by name, by last activity

**State:**
- `balances: PersonBalance[]` — fetched via `useBalances()` hook
- `summary: BalanceSummary` — aggregate KPIs
- `selectedPerson: PersonBalance | null` — for settle-up modal

---

### SettleUpModal (src/components/balances/SettleUpModal.tsx)

**Purpose:** Modal for recording a payment/settlement with a specific person.

**Layout:**
- Header: "Settle Up with [Name]"
- Net balance: "$150.00 — Jane owes you"
- Contributing splits list (collapsible): split name + amount for each
- Settlement type: "Full ($150.00)" / "Partial (enter amount)"
- Payment method: dropdown (Venmo, PayPal, Cash, Bank Transfer, Other)
- Notes: optional text field
- Buttons: "Record Payment" (primary) / "Cancel" (secondary)

**Behavior:**
- "Full" pre-fills the net balance amount; "Partial" shows amount input
- On submit: calls `POST /balances/:userId/settle`
- On success: closes modal, refetches balances, shows success toast
- Validates: amount > 0, amount ≤ net balance

---

## 6.5 Activity Feed Page Specification

### ActivityFeedPage (src/components/activity/ActivityFeedPage.tsx)

**Purpose:** Full-page chronological feed of all user notifications/events.

**Layout:**
- Header: "Activity" + "Mark All as Read" button
- Grouped by date: "Today", "Yesterday", "This Week", "Older"
- Each item: icon (by type) + message + relative time + unread dot
- Click item → navigates to related split/settlement

**Notification Type Icons:**
- `split_created`: Plus circle (blue)
- `payment_received`: Check circle (green)
- `payment_reminder`: Clock (amber)
- `payment_overdue`: Alert triangle (red)
- `recurring_generated`: Refresh (purple)
- `settlement_recorded`: Banknote (green)

### NotificationBell (src/components/activity/NotificationBell.tsx)

**Purpose:** Header icon with unread badge + dropdown preview.

**Behavior:**
- Shows bell icon in app header/nav
- Red badge with unread count (hidden if 0)
- Click → toggles NotificationDropdown (last 5 items + "View All" link)
- Auto-refreshes count every 60 seconds (polling) or via WebSocket

---

## 6.6 People Management Page Specification

### PeopleListPage (src/components/people/PeopleListPage.tsx)

**Purpose:** Main roster management page — view, search, add, edit, and delete people.

**Layout:**
- **Header row:** Title "People" + "Add Person" button (primary, top-right)
- **Search/filter bar:** Text search by name/email, filter by status (active/all)
- **Table view (desktop):** Columns: Avatar+Name, Email, Phone, Splits, Balance, Actions (Edit/Delete)
- **Card view (mobile):** Stacked cards with name, email, split count, balance badge
- **Empty state:** Illustration + "Add your first person to start splitting expenses"

**Interactions:**
- Click "Add Person" → opens PersonForm modal (create mode)
- Click Edit icon → opens PersonForm modal (edit mode, pre-filled)
- Click Delete icon → opens DeletePersonDialog (blocks if unsettled splits)
- Click person row → navigates to PersonDetailPage
- Search filters in real-time (debounced 300ms)

**State:**
- `people: RosterPerson[]` — fetched via `usePeople()` hook
- `searchQuery: string` — controlled input
- `isFormOpen: boolean` — modal state
- `editingPerson: RosterPerson | null` — null for create, populated for edit

---

### PersonForm (src/components/people/PersonForm.tsx)

**Purpose:** Modal form for adding or editing a person in the roster.
...
**Behavior:**
- Create mode: empty fields, submit calls `POST /users`
- Edit mode: pre-filled from existing person, submit calls `PUT /users/:id`
- Validation: inline errors on blur + submit; duplicate email shows server error
- On success: closes modal, refetches people list, shows success toast

---

## 6.7 Analytics Page Specification

### AnalyticsPage (src/components/analytics/AnalyticsPage.tsx)

**Purpose:** Dedicated analytics tab showing spending insights, trends, and balances.

**Layout (desktop — 2-column grid):**
```
┌─────────────────────────────────────────────────────────┐
│  [Date Range Filter]  [Category Filter]  [Person Filter] │
├─────────────────────────────────────────────────────────┤
│ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐    │
│ │Total Split│ │ You Owe  │ │Owed to You│ │ Active   │    │
│ │  $2,450  │ │   $125   │ │   $342   │ │  5 splits │    │
│ └──────────┘ └──────────┘ └──────────┘ └──────────┘    │
├────────────────────────────┬────────────────────────────┤
│   Spending Over Time       │    Category Breakdown      │
│   [Line/Bar Chart]         │    [Donut Chart]           │
├────────────────────────────┼────────────────────────────┤
│   Balance History          │    Top Splitting Partners  │
│   [Line Chart]             │    [Horizontal Bar Chart]  │
├────────────────────────────┴────────────────────────────┤
│          Settlement Status (Paid vs Unpaid)              │
│          [Stacked Bar Chart]                            │
└─────────────────────────────────────────────────────────┘
```
...
- Filters collapsed into a dropdown/drawer

**Chart Specifications:**
- **SpendingChart:** Bar chart with monthly totals; hover shows exact amount + split count
- **CategoryPieChart:** Donut chart with legend; click slice to filter table below
- **BalanceHistoryChart:** Line chart; positive (green) = owed to you, negative (red) = you owe; zero line highlighted
- **TopPartnersChart:** Horizontal bar chart, top 5-10 partners; bar label shows name + amount
- **SettlementStatusChart:** Stacked bar per month; green = settled, amber = partial, red = unsettled

**Data Fetching:**
- `useAnalytics()` hook fetches all endpoints in parallel on mount
- Refetches when filters change (debounced 500ms)
- Loading skeletons shown per chart while data loads
- Error state per widget (doesn't block other widgets)
